...
curl -H "Content-Type: application/json" -X POST -d '{"name": "(4) Top 4 ES6 Components of whatever. ", "url":"http://webapplog.com/es6", "text": ""}'  "http://localhost:3000/posts"
#pause for tester human
echo
read -p "Press any key to continue... " -n1 -s
echo
# Update:
curl -H 'Content-Type: application/json' -X PUT -d '{"name": "3 - Groovier Features Every Developer Must Know", "url":"http://webapplog.com/es6", "text": ""}' "http://localhost:3000/posts/2"
# Get all:
...
curl -H "Content-Type: application/json" -X PUT -d '{"text": "This is a replacement comment"}'  "http://localhost:3000/posts/0/comments/0"
#Delete a comment:
curl -H "Content-Type: application/json" -X DELETE "http://localhost:3000/posts/0/comments/0"
#Delete a post:
curl -H "Content-Type: application/json" -X DELETE "http://localhost:3000/posts/0"
#pause for tester human
echo
read -p "Press any key to continue... " -n1 -s
echo
